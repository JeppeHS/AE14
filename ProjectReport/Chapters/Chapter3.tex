...
\end{lstlisting}
\begin{lstlisting}sortedArray[j] >> (K-i*D)) & MASK_D\end{lstlisting} gets the digit we want to look at by right shifting the value and anding it with a bit mask containing 0s except for the lowest D bits, which is 1s.
...
The algorithm is not generic but assumes that K and D always is K = 32 and D = 8........
